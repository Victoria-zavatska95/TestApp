...
var userCoordinate2D = CLLocationCoordinate2D()
  ...
  self.clearButton.layer.borderColor = myColor.cgColor
        ...
        plugNameTextfield.delegate = self
        addressNameTextfield.delegate = self
        maximumChargingTimeTextfield.delegate = self
        descriptionAddressTextfield.delegate = self
    ...
    // find user's address with Google Geocoding API
    ...
    // check all textfields for characters count
