...
class LoginizationViewController: UIViewController, CLLocationManagerDelegate, UITextFieldDelegate {
    ...
    @IBAction func loginizationAction(_ sender: Any) {
        ...
        if self.currentUser.value(forKey: "DeviceToken") == nil {
            ...
            let validation = ValidationChecking()
            let resultOfChecking = validation.checkingForLoginization(username.text!, usernameLabel, passwordConfirmTextField.text!, passwordLabel)
       
            
            if resultOfChecking.parameters.count == 0 {
                self.usernameLabel.text = resultOfChecking.error["name"]
                ...
                if self.reachability.isReachable {
                    RequestsForUserRegistrationLoginization().loginUser(parameters: parameters, completion: { (status, response, error) in
                        if status {
                            self.performSegue(withIdentifier: "toRequesrForAuthorizationSecond", sender: self)
                        } else {
                            validation.warning(warningLabel: self.errorLabel, warningLabelText: error)
                        }
                    })
                } else {
    ...
    // login user via Facebook account
            ...
            case .success:
                ...
                connection.add(GraphRequest(graphPath: "/me", parameters: ["fields": "email, id, name, first_name, last_name, picture.type(large)"])){ httpResponse, result in
                    ...
                    case .success(let response):
                        ...
                        if self.reachability.isReachable {
                            RequestsForUserRegistrationLoginization().loginUserViaFB(parameters: self.parameters, completion: { (status, json, error) in
                                if status {
